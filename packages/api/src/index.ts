...
export const callingProcedure = orgMemberProcedure.use(
  async ({ context, next }) => {
    const { db, orgId, session } = context;
    const userId = session.user.id;
    const module = MODULE_IDS.CALLING;
    ...
    if (config.mode === "team_based") {
      ...
      if (grantedTeamIds.length === 0) {
        throw new ORPCError("FORBIDDEN", {
          message: "Calling access restricted to granted teams",
          data: {
            code: "MODULE_DISABLED",
            module,
            reason: "team_restricted",
    ...
    if (!userAccess) {
      throw new ORPCError("FORBIDDEN", {
        message: "Calling access restricted to specific users",
        data: {
          code: "MODULE_DISABLED",
          module,
          reason: "user_restricted",

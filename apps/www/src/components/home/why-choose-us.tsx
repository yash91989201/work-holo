...
const logos = [
  ...
  {
    name: "tse",
    sub: "ENERGIE DE CONFIANCE",
    style: "font-bold tracking-tight text-lg",
  },
    ...
    >
      ...
      >
        {allLogos.map((logo, index) => (
          <div
            className="flex min-w-[200px] items-center justify-center rounded-2xl border border-border/30 bg-card/40 px-10 py-7 transition-all duration-300 hover:border-border/60 hover:bg-card/70"
            key={`${logo.name}-${index}`}
          >
            {logo.prefix && (
              <span className="mr-1.5 flex size-7 items-center justify-center rounded-full bg-muted font-bold text-muted-foreground text-xs">
                {logo.prefix}
              </span>
            )}
            <span
            ...
            {logo.suffix && (
              <span className="ml-1 align-top font-bold text-muted-foreground/40 text-xs">
                {logo.suffix}
              </span>
            )}
            {logo.sub && (
              <span className="ml-1.5 text-[9px] text-muted-foreground/30 uppercase leading-none tracking-wider">
                {logo.sub}
              </span>
            )}
          </div>

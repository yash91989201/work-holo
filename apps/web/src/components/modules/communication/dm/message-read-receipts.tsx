...
export function DmMessageReadReceipts({
  ...
  // Filter out own message read receipt
  const recentReaderIds = useMemo(
    () =>
      readReceipts
        .filter((receipt) => !isOwnMessage || receipt.userId !== userId)
        .map((receipt) => receipt.userId),
    [readReceipts, isOwnMessage, userId]
  );
...
function DmMessageReadersDialog({
  readCount,
  messageId,
  conversationId,
}: {
    ...
    <Dialog>
      ...
      <DialogContent className="max-w-md">
        ...
        <ScrollArea className="max-h-96">
          <Suspense fallback={<DmMessageReadersList.Fallback />}>
            <DmMessageReadersList
              conversationId={conversationId}
              messageId={messageId}
            />
          </Suspense>
...
function DmMessageReadersList({
  ...
  const { data: readers } = useSuspenseQuery(
    queryUtils.communication.dm.getAllReaders.queryOptions({
...
function DmMessageReadersListSkeleton() {
  return (
    <ItemGroup>
...
DmMessageReadersList.Fallback = DmMessageReadersListSkeleton;

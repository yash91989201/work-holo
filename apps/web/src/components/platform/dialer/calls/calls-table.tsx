...
type AdminCallsTableInnerProps = {
  organizationId?: string;
};

function AdminCallsTableInner({ organizationId }: AdminCallsTableInnerProps) {
  const { data } = useSuspenseQuery(
    queryUtils.admin.callLogs.list.queryOptions({
      input: { organizationId, limit: 50, offset: 0 },
    })
  );

  const { data: stats } = useSuspenseQuery(
    queryUtils.admin.callLogs.stats.queryOptions({
...
export function AdminCallsTable({ organizationId }: AdminCallsTableInnerProps) {
  return (
    <Suspense
      fallback={
    ...
    >
      <AdminCallsTableInner organizationId={organizationId} />
    </Suspense>
  );
}

AdminCallsTable.Icon = IconPhone;

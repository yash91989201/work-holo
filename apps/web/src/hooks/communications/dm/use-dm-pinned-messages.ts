...
import { buildDmMessageWithAttachments } from "@/lib/communications/dm-message";

export function useVirtualDmPinnedMessages() {
  ...
  const {
    ...
    isFetchingNextPage,
    isLoading,
  } = useDmPinnedMessages({ conversationId });
  ...
  }, [conversationId, isLoading, pinnedMessages.length, virtualizer]);
...
export function useDmPinnedMessages({
  conversationId,
}: {
  ...
  const pinnedMessages = useMemo(() => {
    ...
    for (const { message, sender, attachment } of rowsWithExtra) {
      ...
      if (!entry) {
        entry = buildDmMessageWithAttachments(message, sender);
